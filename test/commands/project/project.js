...
describe('project', function () {
    ...
    const project = proxyquire('../../../lib/commands/project/project', projectStubs);

    describe('create', function (cb) {

        it('should err if project directory already exists', function (done) {
            const name = 'create_err';
            const projPath = path.resolve(tmpDir, name);
        ...
        it('should create a new connect project', function (done) {
            ...
            project.create(name, {framework: 'connect'}, function (err) {
                ...
                const packageJson = path.resolve(projPath, 'package.json');
    ...
    describe('start', function () {
        ...
        it('should pass multiple extra arguments separately', function (done) {
            let options = {nodeArgs: '--harmony --harmony_destructuring'};
            project.start(projPath, options, function (err) {
                should.not.exist(err);
        ...
        it('should combine extra arguments with debug', function (done) {
            let options = {debug: true, nodeArgs: '--harmony'};
            project.start(projPath, options, function (err) {
                should.not.exist(err);
    ...
    describe('verify', function () {
        ...
        describe('with errors', function () {
            ...
            before(function (done) {
                ...
                project.create(name, {framework: 'connect'}, function (err) {
                    should.not.exist(err);
                    const sourceFile = path.join(__dirname, 'badswagger.yaml');
                    const destFile = path.join(projPath, 'api', 'swagger', 'swagger.yaml');
    ...
    describe('generate-test', function () {
        ...
        it('should pass test-module options', function (done) {
            let options = {testModule: 'request'};
            project.generateTest(projPath, options, function (err) {
                
                const packagePath = path.resolve(projPath, 'package.json');
                fs.existsSync(packagePath).should.be.ok();
                const packageJson = require(packagePath);
                /** @namespace packageJson.devDependencies */
        ...
        it('should pass assertion format options', function (done) {
            let options = {assertionFormat: 'expect', force: true};
            project.generateTest(projPath, options, function () {
        ...
        it('should err when given invalid assertion-format options', function (done) {
            let options = {assertionFormat: 'wrong'};
            project.generateTest(projPath, options, function (err) {
                should.exist(err);
                done();
            });
        ...
        it('should generate testing stubs for the project successfully', function (done) {
            let options = {pathName: '.*', force: true};
            project.generateTest(projPath, options, function () {
        ...
        it('should overwrite the existing file with prompt', function (done) {
            ...
            const prevFile = fs.readFileSync(path.resolve(projPath, 'test/api/client/hello-test.js'), {encoding: 'utf8'});
            ...
            project.generateTest(projPath, {}, function () {
        ...
        it('should not overwrite the existing file with prompt', function (done) {
            const swagger = yaml.load(fs.readFileSync(path.join(projPath, 'api/swagger/swagger.yaml'), 'utf8'));
        ...
        it('should overwrite the current file and all following with prompt', function (done) {
            ...
            const prevHello = fs.readFileSync(path.resolve(projPath, 'test/api/client/hello-test.js'), {encoding: 'utf8'});
            const prevTest = fs.readFileSync(path.resolve(projPath, 'test/api/client/test-test.js'), {encoding: 'utf8'});
        ...
        it('should create load tests from myLoadTest.json', function (done) {
            ...
            let options = {loadTest: './myLoadTest.json', force: true};
        ...
        it('should create load tests from load-config.json', function (done) {
            ...
            let options = {loadTest: true, force: true};
